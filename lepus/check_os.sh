...
tags=$8
filter_os_disk=$9
#ip,dbhost,dbport,dbuser,dbpasswd,dbname,community,filter_os_disk,tags
...
if [ "$hostname" != "" ];then
    ...
    let mem_available=$mem_free+$mem_buffered+$mem_cached
    let mem_used=$mem_total-$mem_available
    mem_usage_rate=`awk 'BEGIN{printf "%2d%\n",'$mem_used'/'$mem_total*100'}'`
