...
def get_alarm_web_status():
    sql="""SELECT 
            ...
            web_status.create_time,
            db_servers_web.web_name,
            web_status.attach,
            web_status.is_alarm
            FROM web_status
              LEFT JOIN db_servers_web
                ON web_status.web_id = db_servers_web.id
              LEFT JOIN db_servers_os
                ON db_servers_web.os_id = db_servers_os.id"""
    result=func.mysql_query(sql) 
    if result <> 0:
        for line in result:
            web_status_id=line[0]
            web_id=line[1]
            process_num=int(line[2])
            remark=line[3]
            host=line[4]
            domain=line[5]
            alarm_process=int(line[6])
            alarm_process_mem=int(line[7])
            send_mail=line[8]
            send_mail_to_list=line[9]
            send_sms=line[10]
            send_sms_to_list=line[11]
            ...
            #print "process_num:"+str(process_num)+" alarm_process:"+str(alarm_process)
            if process_num < 1 :
                send_mail = func.update_send_mail_status(web_id,db_type,'connect',send_mail,send_mail_max_count)
                send_sms  = func.update_send_sms_status(web_id,db_type,'connect',send_sms,send_sms_max_count)
                func.add_alarm(web_id,tags,host,web_port,create_time,db_type,'connect','down','critical','Web server down',send_mail,send_mail_to_list,send_sms,send_sms_to_list)
                func.update_web_status(web_status_id)
               
            elif process_num > alarm_process and  alarm_process>0:
                send_mail = func.update_send_mail_status(web_id,db_type,'connect',send_mail,send_mail_max_count)
                send_sms  = func.update_send_sms_status(web_id,db_type,'connect',send_sms,send_sms_max_count)
                func.add_alarm(server_id,tags,host,web_port,create_time,db_type,'process',process_num,'critical','web进程数大于阈值:'+str(alarm_process),send_mail,send_mail_to_list,send_sms,send_sms_to_list)
                func.update_web_status(web_status_id)            
            else:
                func.check_if_ok(web_id,tags,host,web_port,create_time,db_type,'connect','up','web server up',send_mail,send_mail_to_list,send_sms,send_sms_to_list)
                func.update_db_status('connect','1',host,web_port,create_time,'connect','up','ok')
...
def send_alarm():
    sql = "select tags,host,port,create_time,db_type,alarm_item,alarm_value,level,message,send_mail,send_mail_to_list,send_sms,send_sms_to_list,id alarm_id,attach from alarm;"
    result=func.mysql_query(sql)
    if result <> 0:
        ...
        for line in result:
            ...
            if int(send_alarm_mail)==1:
                if send_mail==1:
                    ...
                    result = sendmail.send_mail(mail_to_list,mail_subject,mail_content,attach)
                    if result:
                        send_mail_status=1
                    else:
                        send_mail_status=0
...
def main():
    ...
    monitor_web = func.get_option('monitor_web')
    if monitor_mysql=="1":
